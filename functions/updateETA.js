...
async function updateETA({ registration, customerETA }) {
  ...
  try {
    ...
    let etaTime = moment().tz('Europe/London');

    if (customerETA.match(/\d+\s*(minutes|hours)/i)) {
      const duration = moment.duration(parseInt(customerETA), customerETA.includes('minutes') ? 'minutes' : 'hours');
      ...
      if (match) {
        ...
        etaTime.set({ hour: hours, minute: minutes });
        
        if (etaTime.isBefore(currentTime)) {
          etaTime.add(1, 'day');
        }
      } else {
    ...
    const patchData = {
      records: [{
        id: recordId,
        fields: {
          Current_ETA: etaTime.toISOString() // Use ISO 8601 format
        }
      }],
    ...
    console.error('Error updating ETA:', error.message);
    return { 
